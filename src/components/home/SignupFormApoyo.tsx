...
const formSchema = z.object({
  ...
  group_slot: z.enum(['jueves_19', 'martes_10'], {
...
interface SignupFormApoyoProps {
  onSuccess: () => void;
}

export function SignupFormApoyo({ onSuccess }: SignupFormApoyoProps) {
  ...
  const onSubmit = async (data: FormValues) => {
    const result = await submitSignup({
      form_type: 'apoyo_psicoemocional',
      full_name: data.full_name,
      email: data.email,
      phone: data.phone || undefined,
      group_slot: data.group_slot,
              ...
              <FormControl>
                ...
                >
                  <div className="flex items-center space-x-2">
                    <RadioGroupItem value="jueves_19" id="jueves_19" />
                    <label htmlFor="jueves_19" className="text-sm cursor-pointer">
                      Jueves 19:00 - 21:00 hrs
                    </label>
                  </div>
                  <div className="flex items-center space-x-2">
                    <RadioGroupItem value="martes_10" id="martes_10" />
                    <label htmlFor="martes_10" className="text-sm cursor-pointer">
                      Martes 10:00 - 12:00 hrs
                    </label>
                  </div>
                </RadioGroup>
              </FormControl>
