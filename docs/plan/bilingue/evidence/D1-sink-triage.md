...
>
> **The three labels and the 62-row coverage stand.** Codex round 2 confirmed the floor, the exact
> triage path coverage, the Group 3 exclusion, the #30/#32 moves and every `#n` pointer, and passed
> the mechanical and privacy gates.
>
> **What does not stand is the record model in `D1-surfaces-output.md`**, and therefore every
> *record count* this document quotes. Codex [B2] found the per-origin splitting still inconsistent —
> duplicate terminal-only records, a combined name/role variable record, two distinct download routes
> filed as one — and ruled that the total is not reliable. Overlay §5 has fired: this is the second
> consecutive failure in that category, so the fix is a **changed derivation method**, not another
> hand-edit, and two contract/schema defects need PM resolution first. See
> `reviews/D1b1-r1-review-request.md` §0 (round 3) for the FINDINGS.
>
> Record counts below have therefore been replaced by pointers. Nothing else in this file is
> provisional.
...
| 25 | `src/components/liturgia-builder/ExportPanel.tsx` | `D1b-1` | Its `file download` record and its two split-out locale-date records are in `D1-surfaces-output.md` §4b — **count provisional, see the banner above**. Also the invocation root for `send-music-service-packet` (344) and `send-children-service-packet` (389). **Also owes D1b-2** 5 `toast(` records. |
...
| 30 | `src/components/liturgia/BiblePassageFetcher.tsx` | `no surface` | Outside the boundary, Group 3. **Corrected at round 1 remediation** — round 1 labelled this `D1b-2` while giving as its reason the very chain that excludes it. Its only referrer is `LiturgiaForm.tsx:17,222`, whose only referrer is `OracionesAntifonalesGenerator.tsx:29,390`. 5 `toast(`, all on `/admin/liturgia/oraciones`. The builder calls the same `fetch-bible-passage` edge function through its own component, not through this one. `/usr/bin/grep -rn "'fetch-bible-passage'" src --include='*.ts' --include='*.tsx'` returns three invocation sites, literally `liturgia/BiblePassageFetcher.tsx:68`, `presentation/BibleVerseCreator.tsx:86`, `liturgia-builder/ContextoTransversal.tsx:213`. *(Round 2 cited the unquoted pattern, whose output carries a fourth line — a doc comment at `liturgia/types.ts:130`. Conclusion unchanged, derivation corrected — Codex round-2 [S1].)* |
| 31 | `src/components/liturgia/OracionesAntifonalesGenerator.tsx` | `no surface` | Outside the boundary — **the closest call in this phase**, flagged for the reviewer. Reached only from `src/pages/OracionesAntifonalesPage.tsx`, routed at `appRoutes.tsx:57` as `/admin/liturgia/oraciones`. **The reason below was corrected at round 2's review — Codex [B1].** It previously said this component "inserts into the legacy `liturgias` table, not into the builder's liturgy". That distinction is **false and retracted**: `liturgias` and `liturgia_lecturas` are shared, a row written here at 240 is listed by `listLiturgies` and loadable by `loadLiturgy`, and the readings written at 265 are read by `loadLiturgy:828` into the celebrant PDF. What actually excludes this component is narrower — its **prayer text** goes to `liturgia_oraciones` (281), and `loadLiturgy` queries only `liturgias`, `liturgia_lecturas` and `liturgia_elementos`. The builder also has its own prayer generator (`editors/OracionEditor.tsx:326`). 7 `toast(`. Full table trace in `D1-exclusions.md` §3 Group 3. |
...
| 57 | `supabase/functions/send-children-service-packet/index.ts` | `D1b-1` | Its `email` records are in `D1-surfaces-output.md` §3b — **count provisional, see the banner above**. |
| 58 | `supabase/functions/send-music-service-packet/index.ts` | `D1b-1` | Its `email` records are in `D1-surfaces-output.md` §3a and its `WhatsApp` records in §2, including the `asignacion_servicio` fan-out at 555-570 — **counts provisional, see the banner above**. |
