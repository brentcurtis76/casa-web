...
serve(async (req) => {
  ...
  try {
    // El token vive en los secretos de Supabase: supabase secrets set INSTAGRAM_ACCESS_TOKEN=...
    const accessToken = Deno.env.get("INSTAGRAM_ACCESS_TOKEN");

    if (!accessToken) {
      throw new Error("Falta el token de acceso de Instagram (INSTAGRAM_ACCESS_TOKEN)");
    }
