...
// MUST be deployed public: verify_jwt = false in supabase/config.toml
// ([functions.wa-webhook]) or `supabase functions deploy wa-webhook --no-verify-jwt`.
...
async function processStatusUpdate(
  ...
  // Statuses can arrive out of order; never let an earlier state overwrite a
  // later one (sent must not regress delivered, delivered must not undo failed).
  if (status === "sent") {
    await supabase
      .from("music_notification_logs")
      .update({ status: "sent", sent_at: tsIso })
      .eq("external_id", wamid)
      .in("status", ["queued", "sent"]);
  } else if (status === "delivered") {
    await supabase
      .from("music_notification_logs")
      .update({ status: "delivered", delivered_at: tsIso })
      .eq("external_id", wamid)
      .neq("status", "failed");
  } else if (status === "read") {
...
serve(async (req) => {
  ...
  // Process via waitUntil when available so the runtime keeps the work alive
  // after the 200 is returned; otherwise await inline (Meta tolerates the
  // small response delay, and timeouts just trigger a retry of the same event).
  const work = handleWebhookPayload(rawBody).catch((err) => {
