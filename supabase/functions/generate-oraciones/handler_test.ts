...
//     any Storage access, or the Anthropic provider (`fetch`) are touched
//     — verified with spies.
//   * The guard maps to `oraciones:write` (not the generic liturgy_builder
//     mapping) — verified via the args the handler passes to
//     `checkPermission`.
 ...
 * network call or Storage endpoint).
 */
...
Deno.test("OPTIONS preflight returns 200 with CORS and skips auth guard", async () => {
  ...
  await withFetchSpy(async (fetchSpy) => {
    ...
    // Guard is not consulted for preflight; fetch/Storage is not touched.
...
Deno.test(
  "POST without Authorization returns 401 UNAUTHORIZED and never reads body, hits Storage, or calls provider",
  async () => {
    const { deps: authz, calls } = makeAuthzDeps();
    const handler = createHandler(baseDeps(authz));
    ...
    await withFetchSpy(async (fetchSpy) => {
      ...
      // Fail-closed: no body parse, no fetch (Storage / Anthropic provider),
...
Deno.test(
  ...
  async () => {
    ...
    await withFetchSpy(async (fetchSpy) => {
      ...
      // getUser then checkPermission with the exact resource/action pair.
