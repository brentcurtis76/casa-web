/**
 * CASA Oraciones Antifonales Generator — request handler.
 *
 ...
 * method must pass `requirePermission` with `oraciones:write` BEFORE the
 * handler reads the body, hits Storage, or calls the Anthropic provider.
...
export function createHandler(
  ...
  return async function handler(req: Request): Promise<Response> {
    ...
    // Fail-closed authz. Runs BEFORE req.json(), Storage, or the Anthropic
    // provider. Uses requirePermission with the exact `oraciones:write`
    // resource/action pair (not the generic liturgy_builder mapping).
