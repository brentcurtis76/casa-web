// Integration tests for the refine-story request handler.
//
// T-INT-refine-story covers the fail-closed authz contract:
...
function samplePayload() {
  return {
    currentStory: {
      title: "El Faro de Valparaíso",
      summary: "Un cuento sobre esperanza.",
      characters: [],
      scenes: [{ text: "Escena 1", visualDescription: "desc" }],
...
Deno.test("OPTIONS preflight returns 200 with CORS and skips auth guard", async () => {
  ...
  await withFetchSpy(async (fetchSpy) => {
    const req = new Request("https://edge.test/refine-story", {
      method: "OPTIONS",
    });
...
Deno.test(
  ...
  async () => {
    ...
    await withFetchSpy(async (fetchSpy) => {
      ...
      // Fail-closed: no body parse, no fetch (Anthropic provider), no
      // authz backend calls at all when the credential is missing.
