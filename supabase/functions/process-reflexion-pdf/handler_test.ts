...
//     PDF decoding, Storage access, or the Anthropic provider (`fetch`)
...
function samplePayload() {
  // Small opaque base64 blob — never decoded because the guard rejects
  // first. Kept short so payload size stays trivial.
  return {
    pdfBase64: "JVBERi0xLjQKJcOkw7zDtsOfCg==",
    filename: "reflexion-adviento.pdf",
  };
}
...
Deno.test(
  ...
  async () => {
    ...
    await withFetchSpy(async (fetchSpy) => {
      ...
      // Fail-closed: no body parse (so PDF is never decoded or read from
      // Storage), no fetch (Anthropic provider), no authz backend calls
      // at all when the credential is missing.
...
Deno.test(
  ...
  async () => {
    ...
    await withFetchSpy(async (fetchSpy) => {
      ...
      // This asserts the operation is classified as a WRITE, not a read.
