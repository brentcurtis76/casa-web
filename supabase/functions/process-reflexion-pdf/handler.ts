 ...
 * `serve()`.
 *
 * Auth: OPTIONS is handled before the guard for CORS preflight. Every
 * other method must pass `requireLiturgyWriter` BEFORE the handler reads
 * the request body, decodes the PDF, or calls the Anthropic provider.
 * Reflection PDF processing writes liturgical content, so it maps to
 * `liturgy_builder:write` — not a read operation.
 */
...
export function createHandler(
  ...
  return async function handler(req: Request): Promise<Response> {
    ...
    // Fail-closed authz. Runs BEFORE req.json(), the PDF is decoded, or
    // the Anthropic provider is contacted. Reflection PDF processing
    // writes liturgical content — it is not a read.
