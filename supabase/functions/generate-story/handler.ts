 ...
 * researches the location, analyses images, or calls the Anthropic provider.
...
async function analyzeImagesForVisualDescription(params: {
  ...
  const { name, narrativeRole, referenceImages, googleApiKey } = params;
  const kind = params.kind ?? "landmark";

  if (
    !googleApiKey || !Array.isArray(referenceImages) ||
...
export function createHandler(
  ...
  const { anthropicApiKey, googleApiKey, authzDeps } = deps;
  ...
  return async function handler(req: Request): Promise<Response> {
    ...
    // Fail-closed authz. Runs BEFORE req.json(), Gemini research, image
    // analysis, or the Anthropic provider.
    ...
    try {
      ...
      const locationResearch = await researchLocation(location, googleApiKey);
      ...
      console.log(
        `[generate-story] Escenas: ${story.scenes?.length || 0}, Personajes: ${
      ...
      return new Response(
        JSON.stringify({
          ...
          suggestedDuration: Math.ceil((story.scenes?.length || 12) / 3),
          model: MODEL,
          usage: {
