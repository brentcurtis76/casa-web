...
// The strict provider-output contract, EXTRACTED to `_shared/` at PD-REFINE so
// `refine-story` enforces the same one instead of growing a second copy. A pure
// move: every name below is re-exported unchanged just under this block, so
// this module's public surface and behaviour are exactly what they were.
import {
...
// (Carried from `main`@d79d329 at PREL integration; it replaces this file's
// previous dated `claude-opus-4-5-20251101`.)
...
/**
 * `ProviderOutputReason`, `ProviderOutputError` and their three fixed Spanish
 * messages now live in `_shared/storyContract.ts` (PD-REFINE). Imported and
 * re-exported at the top of this file; the 502 mapping stays here, in the catch.
 */
...
// ---------------------------------------------------------------------------
//
// `validateAndNormalizeStory`, the story types, the scene-count window,
// `STORY_TOOL_SCHEMA` and `describeValidationErrors` MOVED VERBATIM to
// `_shared/storyContract.ts` at PD-REFINE, so `refine-story` enforces the same
// contract. Imported and re-exported at the top of this file; nothing about
// this function's behaviour changed.
...
// `STORY_TOOL_SCHEMA` (the strict `emit_story` schema) moved verbatim to
// `_shared/storyContract.ts` at PD-REFINE; imported at the top of this file.
  ...
  try {
    ...
    const response = await fetchWithRetry(
      'https://api.anthropic.com/v1/messages',
      {
        ...
        body: JSON.stringify({
          model: MODEL,
          // Opus 4.5 did no thinking unless asked; Opus 5 thinks by default, and
          // max_tokens caps thinking + output together — left on, it would eat
          // the budget the strict `emit_story` tool_use needs. Guard carried
          // from `main`@d79d329 at PREL integration; the cap stays at this
          // file's reviewed 16384 rather than main's 8192 (recorded PREL
          // decision, Brent 2026-08-03): a full 15-scene story measures ~4.8k
          // output tokens, billing is per token generated rather than per
          // ceiling, and PD surfaces a `max_tokens` stop_reason as a typed 502
