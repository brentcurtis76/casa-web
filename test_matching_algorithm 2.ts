...
interface Host extends Participant {
...
// The Algorithm
function runMatching(participants: Participant[]) {
    ...
    const unassignedGuests: Participant[] = [];

    // Initialize host tracking
    const hostStatus: Host[] = shuffledHosts.map(host => ({
        ...host,
        currentGuests: 0,
